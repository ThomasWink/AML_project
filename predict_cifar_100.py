...
if model_type == "wrn":
    ...
    models_filenames = [r"%s/WRN-CIFAR100-%d-%d-1.h5" % (files_dir, n, k),
    ...
    models_filenames = [r"%s/DenseNet-CIFAR100-%d-%d-1.h5" % (files_dir, depth, growth_rate),
...
def create_model():
    if model_type == "wrn":
        ...
        return WRN.create_wide_residual_network(init, nb_classes=100, N=args.wrn_N, k=args.wrn_k, dropout=0.00, verbose=False)
    else:
        model_prefix = 'DenseNet-CIFAR100-%d-%d' % (args.dn_depth, args.dn_growth_rate)
        return DN.create_dense_net(nb_classes=100, img_dim=init, depth=args.dn_depth, nb_dense_block=1,
