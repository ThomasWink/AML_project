...
def conv_block(ip, nb_filter, dropout_rate=None, weight_decay=1E-4):
    ...
    x = BatchNormalization(axis=concat_axis, gamma_regularizer=l2(weight_decay), #mode=0 was deleted from original implementation
...
def dense_block(x, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1E-4):
    ...
    for i in range(nb_layers):
        ...
        x = concatenate(feature_list, axis=concat_axis) #mode="concat" removed
...
def create_dense_net(nb_classes, img_dim, depth=40, nb_dense_block=1, growth_rate=12, nb_filter=16, dropout_rate=None,
    ...
    x = Convolution2D(nb_filter, (3, 3), kernel_initializer="he_uniform", padding="same", name="initial_conv2D",
                      kernel_regularizer=l2(weight_decay), use_bias=False)(model_input)
